defmodule Todo.Web do
  ...
  # Convenience endpoint for remembering the other endpoints
  get "/" do
    conn
    |> Plug.Conn.assign(:response, routing_message(conn))
    |> respond
  end

  defp routing_message(conn) do
    scheme = Atom.to_string conn.scheme
    host   = conn.host

    base = case conn.port do
      80   -> "#{scheme}://#{host}"
      port -> "#{scheme}://#{host}:#{inspect port}"
    end

    """
    TODO PATHS
    ==========
    GET  /entries     => curl "#{base}/entries?list=bob&date=20131219"
    POST /add_entry   => curl -d "" "#{base}/add_entry?list=bob&date=20131219&title=Chores"
    """
  end
